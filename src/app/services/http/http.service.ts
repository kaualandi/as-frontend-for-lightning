...
export class HttpService {
  ...
  private getUrl(url: string) {
    if (url.includes("http") || url.startsWith("ws")) return url;
  ...
  handleError = (error: HttpErrorResponse) => {
    ...
    if (
      error.error.detail === "Unable to log in with provided credentials." &&
      this.storage.mySettings.language === "Português"
    ) {
      this.snackbar.error(
        `Erro: ${error.status} - Não é possível fazer login com as credenciais fornecidas.`
      );
      console.log("Não é possível fazer login com as credenciais fornecidas.");
    } else {
